...
impl Superblock {
    ...
    pub fn parse<Dev: Device>(celled_device: &Celled<Dev>) -> Result<Self, Error<Ext2Error>> {
        let mut device = celled_device.lock();

        let superblock_base = device.read_from_bytes::<Base>(Address::from(SUPERBLOCK_START_BYTE), SUPERBLOCK_SIZE)?;
        ...
        if superblock_base.magic != EXT2_SIGNATURE {
            ...
            let superblock_extended_fields = device.read_from_bytes::<ExtendedFields>(
                Address::from(SUPERBLOCK_START_BYTE + usize_to_u64(size_of::<Base>())),
                SUPERBLOCK_SIZE,
            )?;
...
mod test {
    ...
    #[test]
    fn failed_parse() {
        let device = Wrapper::new(vec![0_u8; 2048]);
