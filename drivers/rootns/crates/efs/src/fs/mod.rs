...
//! [`Device`](crate::dev::Device). A [`Filesystem`] is the entry point to the [`File`](crate::fs::file::File)s
//! ([regular files](crate::fs::file::Regular), [directories](crate::fs::file::Directory), ...) of your filesystem
//! through the method [`root`](FilesystemRead::root).
//!
//! The other provided methods, such as [`get_file`](FilesystemRead::get_file), are here to help you read and manipulate
//! easily the content of your filesystem.
//!
//! All the needed methods for [`File`](crate::fs::file::File) manipulation are implemented by other structures linked
//! to a [`Filesystem`]. You can read the module [`mod@file`]'s documentation for more information.
//!
...
//!
//! * create structures for [`File`](crate::fs::file::File), [`Regular`], [`Directory`] and [`SymbolicLink`]. For each
//!   of this structure, create functions allowing to be parsed easily. For [`Fifo`], [`CharacterDevice`],
//!   [`BlockDevice`] and [`Socket`], you can use a simple struct like `struct Socket(File)` as you will likely never
//!   use them directly with this crate
//!
//! * implement the functions allowing to retrieve the [`Regular`], [`Directory`] and [`SymbolicLink`], and the
//!   [`root`](FilesystemRead::root) particularily. For the [`double_slash_root`](FilesystemRead::double_slash_root), if
//!   you don't know what it means, you can just implement it as `self.root()` (and it will very probably be the right
//!   thing to do)
//!
//! * implements all the other functions for the [`Regular`], [`Directory`] and [`SymbolicLink`] structures.
//!
...
use file::{
    Base, BlockDevice, BlockDeviceRead, CharacterDevice, CharacterDeviceRead, DirectoryRead, Fifo, FifoRead, Regular,
    RegularRead, Socket, SocketRead, SymbolicLinkRead,
};
use itertools::{Itertools, Position};

use crate::error::Error;
use crate::fs::error::FsError;
use crate::fs::file::{Directory, SymbolicLink, Type, TypeWithFile};
...
#[cfg(feature = "sfs")]
#[cfg_attr(docsrs, doc(cfg(feature = "sfs")))]
pub mod sfs;
...
pub trait FilesystemRead<Dir: DirectoryRead>
where
    Dir::Regular: RegularRead<FsError = <Dir as Base>::FsError>,
    Dir::SymbolicLink: SymbolicLinkRead<FsError = <Dir as Base>::FsError>,
    Dir::Fifo: FifoRead<FsError = <Dir as Base>::FsError>,
    Dir::CharacterDevice: CharacterDeviceRead<FsError = <Dir as Base>::FsError>,
    Dir::BlockDevice: BlockDeviceRead<FsError = <Dir as Base>::FsError>,
    Dir::Socket: SocketRead<FsError = <Dir as Base>::FsError>,
{
    ...
    ///
    /// If you do not have any idea of what this is, you are probably looking for [`root`](FilesystemRead::root).
    ///
        ...
        Self: Sized,
    {
        ...
        /// bt symbolic links.
        fn path_resolution<FSE: core::error::Error, D: DirectoryRead<FsError = FSE>>(
            fs: &impl FilesystemRead<D>,
            path: &Path,
            mut current_dir: D,
            symlink_resolution: bool,
        ...
        where
            D::Regular: RegularRead<FsError = <D as Base>::FsError>,
            D::SymbolicLink: SymbolicLinkRead<FsError = <D as Base>::FsError>,
            D::Fifo: FifoRead<FsError = <D as Base>::FsError>,
            D::CharacterDevice: CharacterDeviceRead<FsError = <D as Base>::FsError>,
            D::BlockDevice: BlockDeviceRead<FsError = <D as Base>::FsError>,
            D::Socket: SocketRead<FsError = <D as Base>::FsError>,
        {
...
/// A filesystem.
pub trait Filesystem<Dir: Directory>: FilesystemRead<Dir>
where
    Dir::Regular: Regular<FsError = <Dir as Base>::FsError>,
    Dir::SymbolicLink: SymbolicLink<FsError = <Dir as Base>::FsError>,
    Dir::Fifo: Fifo<FsError = <Dir as Base>::FsError>,
    Dir::CharacterDevice: CharacterDevice<FsError = <Dir as Base>::FsError>,
    Dir::BlockDevice: BlockDevice<FsError = <Dir as Base>::FsError>,
    Dir::Socket: Socket<FsError = <Dir as Base>::FsError>,
{
    /// Creates a new file with the given `file_type` at the given `path`.
    ///
    /// # Errors
    ///
    /// Returns a [`Error::IO`] if the device could not be read/written.
    ///
