...
impl<'path> const Deref for UnixStr<'path> {
...
impl<'path> UnixStr<'path> {
    ...
    /// use efs::path::UnixStr;
    ///
    /// assert!(UnixStr::new("//home/foo").unwrap().starts_with_two_slashes());
    ///
    /// assert!(!UnixStr::new("foo/bar").unwrap().starts_with_two_slashes());
    /// assert!(!UnixStr::new("///home/foo").unwrap().starts_with_two_slashes());
    /// ```
    #[must_use]
    pub fn starts_with_two_slashes(&self) -> bool {
...
pub struct Path<'path> {
    /// Inner representation of a path by a [`UnixStr`].
    name: UnixStr<'path>,
}
...
impl<'path> Path<'path> {
    ...
    /// # Errors
    ///
    /// Returns a [`PathError`] if the given UTF-8 sequence is not a valid [`UnixStr`].
    ///
    /// # Safety
    ///
    /// Must ensure that the given slice is a valid UTF-8 sequence.
    pub unsafe fn from_ut8_slice(slice: &[u8]) -> Result<Self, PathError> {
        // SAFETY: it is assumed that the given slice is a valid UTF-8 sequence
        let binding = unsafe { &String::from_utf8_unchecked(slice.to_vec()) };
        Ok(Self::new(UnixStr::from_str(binding)?))
    }
    ...
    #[allow(clippy::missing_const_for_fn)] // const Deref is not implemented for Cow<'_, str>
    #[must_use]
    pub fn len(&self) -> usize {
        (*self.as_unix_str()).len()
    }
    ...
    /// Returns the file name of the final component of this path, **if it's not a slash**.
    ///
...
impl<'path> Iterator for &mut Components<'path> {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        let binding = unsafe { &String::from_utf8_unchecked(self.path.to_vec()) };
        let Ok(path) = Path::from_str(binding) else {
