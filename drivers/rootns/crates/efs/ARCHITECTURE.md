...
`src/fs/file.rs` contains all the traits to manipulate the different types of files that one can find in a POSIX filesystem. In particular, `File`, `Regular`, `Directory` and `SymbolicLink` are the one to look at at the beginning.
